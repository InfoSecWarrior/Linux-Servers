...
In this structure:
- **dc=armour,dc=local** represents the domain (root of the tree).
...
**Example DN:**
```
dn: uid=armour101,ou=Students,dc=armour,dc=local
```
This DN indicates that user "armour101" is located in the "Students" organizational unit within the "armour.local" domain.
...
- RDN: `uid=armour101`
- Parent: `ou=Students,dc=armour,dc=local`
...
```ldif
dn: ou=Training,ou=Departments,dc=armour,dc=local
objectClass: top
...
dn: ou=Students,dc=armour,dc=local
objectClass: top
objectClass: organizationalUnit
ou: Students

dn: ou=Instructors,dc=armour,dc=local
objectClass: top
objectClass: organizationalUnit
ou: Instructors
...
# Groups
dn: cn=cybersec,ou=Groups,dc=armour,dc=local
objectClass: top
objectClass: posixGroup
cn: cybersec
gidNumber: 2001

dn: cn=networking,ou=Groups,dc=armour,dc=local
objectClass: top
objectClass: posixGroup
cn: networking
gidNumber: 2002
...
# Instructor entries
dn: uid=trainer01,ou=Instructors,dc=armour,dc=local
objectClass: top
objectClass: person
objectClass: organizationalPerson
objectClass: inetOrgPerson
...
```bash
ldapadd -x -D "cn=admin,dc=armour,dc=local" -W -f structure.ldif
```
