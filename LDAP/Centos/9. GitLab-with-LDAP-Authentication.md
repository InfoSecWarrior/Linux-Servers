...
The solution is to disable GitLab's Nginx and configure your existing Apache server to act as a "reverse proxy." Apache will handle all incoming traffic on ports 80 and 443 and intelligently forward requests to the correct application:
*   Requests for `gitlab.armour.local` will be sent to GitLab's internal web server.
*   Requests for `wordpress.armour.local` will be served by Apache directly.

---

## **Step 1: Configure GitLab to Disable Nginx**
...
## **Step 2: Create Apache VirtualHost for GitLab**
...
## **Step 3: Update Existing Apache VirtualHost (WordPress)**
...
## **Step 4: Create SSL Certificates for Apache**
